...
Future<void> _loadRecommendations() async {
  ...
  try {
    ...
    if (user != null && _currentPosition != null) {
      ...
      setState(() {
        _eventosRecomendados = recommendations.where((r) => r['type'] == 'evento').toList();
        _lugaresRecomendados = recommendations.where((r) => r['type'] == 'lugar').toList();
      });
    }
  } catch (e) {
    print('Error: $e');
    ...
    child: Opacity(
      ...
      child: Transform.scale(
        ...
        child: Card(
          ...
          child: Container(
            decoration: isAttractive ? BoxDecoration(
              borderRadius: BorderRadius.circular(12),
            ...
            child: ListTile(
              leading: Stack(
                children: [
                  CircleAvatar(
                    backgroundColor: recommendation['type'] == 'evento' ? 
                      (isAttractive ? Colors.blue : Colors.blue.withOpacity(0.6)) : 
                      (isAttractive ? Colors.green : Colors.green.withOpacity(0.6)),
                    child: Icon(
                      recommendation['type'] == 'evento' ? Icons.event : Icons.location_on,
                      color: Colors.white,
              ...
              title: Text(
                ...
                style: TextStyle(
                  fontWeight: isAttractive ? FontWeight.bold : FontWeight.normal,
              ...
              subtitle: Column(
                ...
                children: [
                  Text(
                    ...
                    style: TextStyle(
                      color: isAttractive ? Colors.blue[700] : Colors.grey[600],
                      fontWeight: isAttractive ? FontWeight.w600 : FontWeight.normal,
                    ),
                  ),
                  Text(
              ...
              onTap: () => _showItemDetails(recommendation['item'], recommendation['type']),
            ),
          ),
        ),
      ),
              ...
              : TabBarView(
                  ...
                  children: [
                    ...
                    _eventosRecomendados.isEmpty
                        ? const Center(child: Text('No hay eventos recomendados'))
                        : ListView.builder(
                            itemCount: _eventosRecomendados.length,
                            itemBuilder: (context, index) =>
                                _buildRecommendationCard(_eventosRecomendados[index]),
                          ),
                    // Tab de Lugares
                    _lugaresRecomendados.isEmpty
                        ? const Center(child: Text('No hay lugares recomendados'))
                        : ListView.builder(
                            itemCount: _lugaresRecomendados.length,
                            itemBuilder: (context, index) =>
                                _buildRecommendationCard(_lugaresRecomendados[index]),
                          ),
                    // Tab de Favoritos
